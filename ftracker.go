...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	calories := (runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * float64(weight) / mInKm * duration * minInH
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return walkingCaloriesWeightMultiplier*float64(weight) + ((math.Pow((meanSpeed(action, duration)/kmhInMsec), 2)/height)*walkingSpeedHeightMultiplier*weight)*duration*minInH
}
